...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(os.path.join(LOG_DIR, "low_seats_alert.log")),
    ],
)
...
def run():
    ...
    logger.info(f"Checking flights with seats_available < {LOW_SEAT_THRESHOLD}")
    ...
    try:
        ...
        low_seat_flights = [
            f for f in all_flights if f.seats_available < LOW_SEAT_THRESHOLD
        ]
        ...
        if not low_seat_flights:
            ...
            logger.warning(
                f"ALERT: {len(low_seat_flights)} flight(s) have fewer than "
