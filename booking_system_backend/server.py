...
def export_booking_ics(booking_id: int) -> str:
    """Export a booking as an iCalendar (.ics) string (RFC 5545).
    Returns the iCalendar text for the booking, or raises an error if not found."""
    db = SessionLocal()
    try:
        result = booking.generate_booking_ics(db, booking_id)
        if isinstance(result, ErrorResponse):
            raise Exception(result.details or result.error)
        return result
    finally:
