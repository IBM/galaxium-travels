...
class TestIcsExport:
    """Test generate_booking_ics service function."""

    def test_export_ics_success(self, db_session):
        """Test successful ICS generation returns valid iCalendar string."""
        db_session.add(User(name="Test User", email="test@example.com"))
        db_session.add(Flight(
            origin="Earth",
            destination="Mars",
        ...
        booking_obj = db_session.query(Booking).first()
        result = booking.generate_booking_ics(db_session, booking_obj.booking_id)
    ...
    def test_export_ics_booking_not_found(self, db_session):
        ...
        result = booking.generate_booking_ics(db_session, 999)
        assert isinstance(result, ErrorResponse)
        assert result.error_code == "BOOKING_NOT_FOUND"
