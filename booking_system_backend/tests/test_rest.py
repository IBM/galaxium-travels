...
class TestIcsEndpoint:
    """Test GET /bookings/{id}/export.ics endpoint."""

    def test_export_ics_returns_calendar_file(self, client, db_session, sample_user_data):
        """Test that the endpoint returns a text/calendar file for a valid booking."""
        user_response = client.post("/register", json=sample_user_data)
        user_id = user_response.json()["user_id"]
    ...
    def test_export_ics_not_found(self, client, db_session):
        """Test that the endpoint returns 404 for an unknown booking id."""
