...
class TestExportICSEndpoint:
    ...
    def test_export_ics_success(self, client, db_session, sample_user_data):
        """Test successful ICS export returns valid iCalendar content."""
        # Register user
        user_response = client.post("/register", json=sample_user_data)
        user_id = user_response.json()["user_id"]

        # Seed flight and booking
        db_session.add(Flight(
            origin="Earth",
            destination="Mars",
            departure_time="2099-01-01 09:00",
        ...
        assert "Earth" in response.text
        assert "Mars" in response.text

    def test_export_ics_not_found(self, client, db_session):
        """Test ICS export with non-existent booking returns 404."""
